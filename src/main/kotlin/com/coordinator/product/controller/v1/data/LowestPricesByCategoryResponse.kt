...
class LowestPricesByCategoryResponse(lowestPricesByCategory: LowestPricesByCategory) {
    @JsonProperty("카테고리")
    val products: List<LowestPriceResponse> =
        lowestPricesByCategory.lowestPriceProducts.map(LowestPricesByCategoryResponse::LowestPriceResponse)

    @JsonProperty("총액")
    @JsonSerialize(using = BigDecimalPriceSerializer::class)
    val totalPrice: BigDecimal = lowestPricesByCategory.totalPrice

    class LowestPriceResponse(lowestPrices: LowestPrices) {
        @JsonProperty("카테고리")
        val category: Category = lowestPrices.category
        ...
        * */
        @JsonProperty("브랜드")
        val brand: List<String> = lowestPrices.brandNames

        @JsonProperty("가격")
        @JsonSerialize(using = BigDecimalPriceSerializer::class)
        val price: BigDecimal = lowestPrices.price
    }
}
